...
def vis_label(dataroot, version, xbound, ybound, thickness, angle_class):
    ...
    dataset = HDMapNetSemanticDataset(nusc, version=version, dataroot=dataroot, data_conf=data_conf, is_train=False)
    ...
    for idx in tqdm.tqdm(range(dataset.__len__())):
        ...
        instance_mask = instance_mask.numpy().sum(0).astype('uint8')
