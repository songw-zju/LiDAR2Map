...
class HDMapNetDataset(Dataset):
    def __init__(self, nusc, version, dataroot, data_conf, is_train):
...
class HDMapNetSemanticDataset(HDMapNetDataset):
    def __init__(self, nusc, version, dataroot, data_conf, is_train):
        super(HDMapNetSemanticDataset, self).__init__(nusc, version, dataroot, data_conf, is_train)
...
def semantic_dataset(version, dataroot, data_conf, bsz, nworkers, distributed, parser_name):
    ...
    val_dataset = parser(nusc, version, dataroot, data_conf, is_train=False)

    if distributed:
        train_sampler = torch.utils.data.distributed.DistributedSampler(train_dataset)
        val_sampler = None
        train_loader = torch.utils.data.DataLoader(train_dataset, batch_size=bsz, sampler=train_sampler, num_workers=nworkers, drop_last=True)
        val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=bsz, shuffle=False, num_workers=nworkers)
        # val_sampler = torch.utils.data.distributed.DistributedSampler(val_dataset)
        # val_loader = torch.utils.data.DataLoader(val_dataset, batch_size=bsz, sampler=val_sampler, num_workers=nworkers)
    else:
        train_sampler = None
        val_sampler = None
