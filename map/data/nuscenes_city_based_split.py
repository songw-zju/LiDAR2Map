...
def create_city_based_splits_scenes(nusc, verbose: bool = False) -> Dict[str, List[str]]:
    all_train_val_scenes = train + val

    scene_to_city = {scene['name']: nusc.get('log', scene['log_token'])['location'] for scene in nusc.scene}

    city_based_train = []
    city_based_val = []

    for scene in all_train_val_scenes:
        if scene_to_city[scene] == 'singapore-queenstown' or scene_to_city[scene] == 'singapore-hollandvillage':
            city_based_train.append(scene)
        elif scene_to_city[scene] == 'singapore-onenorth' or scene_to_city[scene] == 'boston-seaport':
            city_based_val.append(scene)
        else:
            raise ValueError

    scene_splits = {'train': city_based_train, 'val': city_based_val, 'test': test}
    if verbose:
...
if __name__ == '__main__':
    nusc = NuScenes(version='v1.0-mini', dataroot='/home/ws/data/nuscenes', verbose=False)
    create_city_based_splits_scenes(nusc, verbose=True)
